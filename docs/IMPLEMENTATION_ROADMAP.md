...
8. Governed semantic runtime for the ten design-authorized semantic/hybrid rules.
...
- **M6 - Governed semantic output and categorical-confidence contract: currently implemented.**
  RULE002, RULE004, and RULE008 now use item-level structured output, backend-derived
  High/Medium/Low confidence, same-analysis evidence validation, and controlled recommendation
  handling.
- **M7 - Semantic RULE001/RULE007 relationships and deterministic RULE005/RULE009 coverage:
  currently implemented.** Derived mappings reference confirmed IDs and never overwrite source
  evidence; coverage is aggregated deterministically.
...
semantic contract. Documentation must still distinguish the approved production target from the
local development baseline used for offline demonstrations.
...
- The currently implemented semantic runtime includes all ten design-authorized semantic/hybrid
  rules. Outputs are strict, versioned, item-level, evidence-linked, backend-confidence-derived,
  deterministically validated, and provenance-persisted. `RULE006` remains intentionally partial:
