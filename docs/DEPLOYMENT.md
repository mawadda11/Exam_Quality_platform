...
# Local OCR and extraction configuration

The backend image installs Tesseract. Native deployments must install the
Tesseract executable and required Arabic/English language packs. Set
`EXAM_OCR_PROVIDER=tesseract`; no OCR credential is required and no cloud OCR
service is used.

Extraction-only Gemini structure assistance is disabled by default. After
privacy approval, set `EXTRACTION_AI_ENABLED=true`,
`EXTRACTION_AI_PROVIDER=gemini`, the approved `EXTRACTION_AI_MODEL`,
`EXTRACTION_AI_VALIDATION_RETRIES`, `EXTRACTION_AI_PAGE_DPI`,
`EXTRACTION_AI_MAX_PAGES_PER_DOCUMENT`, `EXTRACTION_AI_CACHE_ENABLED`,
`EXTRACTION_AI_TARGETED_OCR_ENABLED`,
`EXTRACTION_AI_CANDIDATE_MIN_CONFIDENCE`, and `GEMINI_API_KEY`. These settings
are independent from academic `AI_PROVIDER`/`AI_MODEL`. Enabling extraction AI
sends selected complete page images and normalized local extraction evidence;
deployment privacy approval must explicitly cover this transfer. Validated
responses are cached with owner-scoped upload artifacts by default to conserve
quota and are removed during analysis deletion. Automated tests use fake
clients and never contact Gemini.
