...
export function getCourseSpecificationPdf(analysisId: string): Promise<Blob> {
  return apiGetBlob(`/analyses/${analysisId}/files/tp153/content`)
}
...
export async function getCourseSpecificationPageImage(
  analysisId: string,
  pageNumber: number,
  geometry: ExtractionReviewGeometry | null,
  options: { crop?: boolean; padding?: number; dpi?: number } = {},
  ...
  const suffix = query.size ? `?${query.toString()}` : ''
  const response = await apiGetBlobResponse(
    `/analyses/${analysisId}/files/tp153/pages/${pageNumber}/image${suffix}`,
  )
  return {
    blob: response.blob,
    pageWidth: Number(response.headers.get('X-PDF-Page-Width') ?? 612),
