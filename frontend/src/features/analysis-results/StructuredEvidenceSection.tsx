...
function relationshipCandidates(relationship: MaterialRelationshipView): PhysicalMaterialView[] {
  if (relationship.exactCandidates.length > 0) return relationship.exactCandidates
  return relationship.contextCandidates
}

function isContextualReference(relationship: MaterialRelationshipView): boolean {
  return (
    relationship.reference.normalized_target_label.endsWith(':unlabeled') ||
    relationship.reference.association_candidates.some(
      (candidate) => candidate.selected && !candidate.exact_label_match,
    )
  )
}

function relationshipResultLabel(relationship: MaterialRelationshipView): string {
  if (isContextualReference(relationship)) return 'Faculty review only'
  return RESULT_LABELS[relationship.result]
}
...
function relationshipReason(
  ...
  if (isContextualReference(relationship)) {
    return t(
    ...
    case 'ambiguous':
      return t(
        '{question} refers to {reference}, but {count} distinct materials use that label.',
        {
          question: questionReference,
          reference: relationship.reference.target_label,
          count: relationshipCandidates(relationship).length,
        },
      )
    case 'nearby':
      return t(
            ...
            <dl className="material-reference-metadata material-drawer-metadata-card">
              <div>
                ...
                <dd>
                  ...
                  >
                    {t(relationshipResultLabel(relationship))}
                  </span>
                </dd>
              </div>
              <div>
                ...
                <dd>{t(
                  isContextualReference(relationship)
                    ? 'Contextual supporting-material reference'
  ...
  const presentation = useMemo(() => {
    ...
    const referencedMaterialIds = new Set(
      references.flatMap((relationship) => [
        ...
        ...relationship.contextCandidates.map((candidate) => candidate.material.id),
            ...
            {[
              ...
              .map((row) => {
                if (row.kind === 'reference') {
                  ...
                  const matchedText =
                    relationship.result === 'ambiguous'
                      ? t('{count} possible matches', { count: relationshipCandidates(relationship).length })
                      : relationship.matchedMaterial
