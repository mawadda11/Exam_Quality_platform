...
import { getExamPdf } from '../../api/analyses'
...
export function ExamPdfPreview({
  ...
  const pdfRequestKey = `${analysisId}:${locale}`
  const pageRequestKey = `${analysisId}:${pageNumber}:${pageLoadNonce}:${locale}`
      ...
      .catch((loadError: unknown) => {
        if (active) {
          setPdfResult({
            ...
            url: null,
            error: localizeInterfaceError(loadError, locale, t, 'Could not load exam PDF'),
          })
        }
      })
    return () => {
  ...
  }, [analysisId, locale, pdfRequestKey, t])

  useEffect(() => {
    let active = true
    void loadCachedExamImage(analysisId, pageNumber, null, PAGE_IMAGE_OPTIONS)
      .then((response) => {
        if (!active) return
        setPageResult({
  ...
  }, [analysisId, locale, pageLoadNonce, pageNumber, pageRequestKey, t])
  ...
  function retryPageImage(): void {
    invalidateCachedExamImage(analysisId, pageNumber, null, PAGE_IMAGE_OPTIONS)
  ...
  return (
    <aside
      ...
      aria-label={t('Original examination PDF')}
      tabIndex={-1}
    >
      <div className="exam-pdf-preview__toolbar">
        ...
        {onGeometryChange && (
          ...
          >
            {selectionMode ? t('Cancel area adjustment') : t('Adjust question area')}
          </Button>
        )}
      </div>
      {focusRequest > 0 && (
      ...
      {selectionMode && (
        <p className="exam-pdf-preview__selection-help">
          {t('Drag over the complete original question, including its table, figure, or answer area.')}
        </p>
      )}
      ...
      {isPageLoading && !visibleAsset && !pageError && (
      ...
      {pageError && pdfUrl && (
        <div className="exam-pdf-preview__fallback">
          ...
          <object
            ...
            aria-label={`${t('Original examination PDF')} — ${t('Page')} ${pageNumber}`}
          >
        ...
        <div className="exam-pdf-preview__viewport">
          ...
          >
            <img
              ...
              alt={`${t('Original examination PDF')} — ${t('Page')} ${pageNumber}`}
              draggable={false}
