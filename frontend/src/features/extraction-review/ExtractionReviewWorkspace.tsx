...
function addedQuestionIssues(snapshot: ExtractionReviewSnapshot): string[] {
  ...
  for (const question of snapshot.questions) {
    if (
      !question.included ||
      (!isManualQuestion(question) && !isStructuredTemplateQuestion(question))
    ) {
      continue
    }
    if (
...
function editableQuestionText(value: string, numberLabel: string): string {
  if (!numberLabel.trim()) return value
  const escaped = numberLabel.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  const pattern = new RegExp(
    `^\\s*(?:question\\s*)?(?:${escaped})\\s*(?:[_:.)-]+|\\s+)\\s*`,
    'i',
  )
        ...
        return (
          ...
          >
            ...
            <div className="review-question-summary">
              <p dir="auto" className="review-question-summary__text">
                <bdi>{editableQuestionText(item.question_text, item.number_label)}</bdi>
              </p>
              <div className="review-question-summary__meta">
                <span>{t(item.question_type ?? 'unknown')}</span>
                <span>
                  {item.marks === null
                  ...
                  <div className="review-form-grid review-form-grid--primary">
                    ...
                    <label className="review-field-wide">
                      ...
                      <textarea
                        dir="auto"
                        rows={4}
                        value={editableQuestionText(item.question_text, item.number_label)}
                        disabled={disabled || !item.included}
                        onChange={(event) =>
...
export function ExtractionReviewWorkspace({
  ...
  async function importStructuredTemplate(file: File): Promise<void> {
    ...
    try {
      ...
      setDraft((current) =>
        current ? applyStructuredQuestionRows(current, rows, reviewRecordId) : current,
      )
      const first = rows[0]
      if (first) {
      ...
      setNotice(t('Structured question template imported. Review every question before saving.'))
    } catch (importError) {
  ...
  function importPastedQuestionText(): void {
    ...
    try {
      ...
      setNotice(t('Pasted questions imported. Review every question before saving.'))
    } catch (importError) {
      const message =
  ...
  const tabs: TabItem<ReviewTab>[] = [
    ...
    {
      id: 'structured',
      label: `${t('Linked Supporting Context')} (${
  ...
  function removeManualQuestion(sourceRecordId: string): void {
    ...
    setDraft((current) => {
      if (!current) return current
      const source = current.questions.find((item) => item.source_record_id === sourceRecordId)
      if (!source || !isManualQuestion(source)) return current
      return {
        ...current,
  ...
  async function handleSave(): Promise<void> {
    ...
    if (hasIncompleteAddedQuestions) {
      setError(t(preparationMode === 'structured_template'
  ...
  function requestConfirmation(): void {
    if (!review || isDirty || !review.can_confirm || hasIncompleteAddedQuestions) return
    if (flaggedQuestionIds.length > 0 || technicalWarningCount > 0) {
  ...
  return (
    <div className="extraction-review-workspace">
      ...
      <section className="review-faculty-summary" aria-labelledby="review-faculty-summary-title">
        <div>
          <h2 id="review-faculty-summary-title">{t('Review summary')}</h2>
          <div className="review-faculty-summary__counts">
            ...
            <span><strong>{flaggedQuestionIds.length}</strong> {t('questions needing review')}</span>
            <span><strong>{review.confirmation_blockers.length}</strong> {t('true blockers')}</span>
          </div>
          <p>
            {t('Confirmation availability')}:{' '}
            <strong>
              {review.can_confirm && !isDirty && !hasIncompleteAddedQuestions
                ? t('Available now')
                : isDirty
                  ? t('Available after saving this revision')
                  : t('Not available yet')}
            </strong>
          </p>
        </div>
        {flaggedQuestionIds.length > 0 && (
          <Button variant="secondary" onClick={reviewFlaggedQuestions}>
            {t('Review flagged questions')}
          </Button>
        )}
      </section>
      ...
      {preparationMode === 'assisted_pdf' && (
        <Alert variant="info" title={t('Assisted PDF extraction')}>
          {t('Automatic extraction is a proposal only. Correct incomplete text and question regions before confirmation.') }
        </Alert>
      )}
      ...
      {(extractionWarningGroups.length > 0 || groupedReviewWarnings.length > 0) && (
      ...
      {hasIncompleteAddedQuestions && (
        <Alert variant="warning" title={t('Complete the added questions before saving')}>
          {t(preparationMode === 'structured_template'
      ...
      <div className="review-sticky-actions">
        ...
        <div className="review-action-buttons">
          <Button
            variant="secondary"
            disabled={!review.can_edit || !isDirty || hasIncompleteAddedQuestions}
            isLoading={isSaving}
          ...
          <Button
            disabled={!review.can_confirm || isDirty || hasIncompleteAddedQuestions}
            isLoading={isConfirming}
            loadingLabel={t('Confirming…')}
