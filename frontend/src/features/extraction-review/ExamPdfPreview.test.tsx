...
describe('ExamPdfPreview', () => {
  ...
  it('offers a selectable PDF copy mode without changing the editable destination', async () => {
    render(
      <ExamPdfPreview
        analysisId="analysis-1"
        pageNumber={1}
    ...
    await screen.findByRole('img', { name: 'Original examination PDF — Page 1' })
    ...
    const selectablePdf = document.querySelector('object[aria-label="Selectable examination PDF — Page 1"]')
    expect(selectablePdf).not.toBeNull()
    expect(selectablePdf?.getAttribute('data')).toContain('blob:protected-exam#page=1')
  })
