...
function buildQuestionRows(
  ...
  createId: () => string,
  extractionMethod: 'structured_template' | 'pasted_review',
): {
  questions: ExtractionReviewQuestion[]
  evidence: ExtractionReviewEvidence[]
  questionOptions: ExtractionReviewQuestionOption[]
} {
  ...
  return { questions, evidence, questionOptions }
}
...
export function applyStructuredQuestionRows(
  ...
  const { questions, evidence, questionOptions } = buildQuestionRows(
    rows,
    createId,
    'structured_template',
  )
...
/**
 * Replace the reviewer-visible automatic draft with pasted questions without
 * changing the analysis preparation mode or deleting immutable machine source
 * records. Machine questions remain in the revision as excluded audit records,
 * while the pasted rows become the included human-reviewed question list.
 */
export function applyPastedQuestionRows(
  snapshot: ExtractionReviewSnapshot,
  rows: StructuredQuestionRow[],
  createId: () => string = () => crypto.randomUUID(),
): ExtractionReviewSnapshot {
  ...
  return {
    ...snapshot,
    questions: [
      ...snapshot.questions.map((item) => ({ ...item, included: false })),
      ...questions,
    ],
    question_options: [
      ...(snapshot.question_options ?? []).map((item) => ({ ...item, included: false })),
      ...questionOptions,
    ],
    question_blanks: (snapshot.question_blanks ?? []).map((item) => ({
      ...item,
      included: false,
    })),
    evidence: [
