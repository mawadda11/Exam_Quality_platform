...
describe('simplified question review', () => {
  it('keeps advisory reconciliation visible without disabling confirmation or requiring resolution', async () => {
    ...
    expect(screen.getByText('questions needing review').closest('span')).toHaveTextContent(
      '1 questions needing review',
    )
    expect(screen.getByRole('button', { name: 'Review flagged questions' })).toBeEnabled()
    ...
    expect(screen.getByRole('dialog', { name: 'Continue with review recommendations?' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Continue to analysis' })).toBeEnabled()
    expect(screen.getByRole('button', { name: 'Return to review' })).toBeEnabled()
    expect(screen.getByText('Technical extraction details (20)').closest('details'))
      .not.toHaveAttribute('open')
    expect(container.querySelectorAll('.review-reconciliation-warning')).toHaveLength(1)
    expect(screen.getByText('Check the section and child marks against the PDF.')).toBeInTheDocument()
  })
