import { getCourseSpecificationPageImage, getExamPageImage, type ExamPageImageResponse } from '../../api/analyses'
...
async function requestPdfImage(
  analysisId: string,
  ...
  const request = sourceDocument === 'tp153'
    ? getCourseSpecificationPageImage
    : getExamPageImage
  try {
    return await request(analysisId, pageNumber, geometry, options)
  } catch (error) {
    if (!shouldRetryImageRequest(error)) throw error
    await new Promise((resolve) => window.setTimeout(resolve, 250))
    return request(analysisId, pageNumber, geometry, options)
  }
}
...
export function examImageCacheKey(
  ...
  geometry: ExtractionReviewGeometry | null,
  options: ImageOptions = {},
  sourceDocument: UploadedFileType = 'exam',
): string {
  return [
    analysisId,
    sourceDocument,
...
export function loadCachedExamImage(
  ...
  const key = examImageCacheKey(analysisId, pageNumber, geometry, options, sourceDocument)
  const existing = imageCache.get(key)
  if (existing) return existing

  const request = requestPdfImage(analysisId, sourceDocument, pageNumber, geometry, options)
    .then((response) => {
