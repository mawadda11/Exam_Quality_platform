...
export function presentFindingExplanation(finding: FindingResponse, locale: Locale): string {
  // Dynamic academic reasoning is intentionally preserved in the validated
  // source language. The Arabic locale translates UI chrome and governed
  // static labels only; it does not machine-translate generated findings.
  if (finding.explanation.trim()) {
    return presentCourseSpecificationTerminology(finding.explanation)
  }
  if (locale !== 'ar') return presentCourseSpecificationTerminology(finding.explanation)
