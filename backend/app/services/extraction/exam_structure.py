...
_MCQ_CUE = re.compile(r"\b(choose|select|multiple choice|اختر)\b", re.IGNORECASE)
...
def _strip_marks_from_question_text(
    ...
    return [
        replace(question, text=strip_marks_annotations(question.text))
        for question in questions
    ]
...
def _attach_spatial_marks(
    ...
    used_ids = {
        source_line_id for question in result for source_line_id in question.source_line_ids
    }
    available = [
        line
        for line in source_lines
...
def _attach_unassigned_blank_lines(
    ...
    result = list(questions)
    for line in source_lines:
        if _is_answer_space_line(line.original_text) or is_code_line(line.original_text):
            continue
        if any(
        ...
        candidates = [
            (index, question)
            for index, question in enumerate(result)
            if question.page_number == line.page_number and question.geometry is not None
        ]
        if not candidates:
            continue
        line_center = (line.geometry.top + line.geometry.bottom) / 2
        ...
        assert question.geometry is not None
        question_center = (question.geometry.top + question.geometry.bottom) / 2
        if abs(line_center - question_center) > 144:
            continue
        local_key = question.local_key or f"P{question.page_number}-Q{question.sequence}"
        existing = list(question.blanks)
        existing.extend(
...
def _question_is_before_material(
    ...
    return question.geometry.bottom <= material.geometry.top + 4
    ...
    def parse(
        ...
        *,
        ...
        for index, question in enumerate(sorted_questions):
            ...
            structured_option_count = len(options)
            for option_index, line in enumerate(option_lines, start=1):
                matched = _OPTION.match(line.original_text)
                assert matched is not None
                options.append(
                    ExtractedQuestionOption(
                        local_key=f"{local_key}-O{structured_option_count + option_index}",
                        question_local_key=local_key,
                        option_label=matched.group(1),
                        option_text=matched.group(2).strip(),
                        sequence=structured_option_count + option_index,
                        page_number=line.page_number,
            ...
            cleaned_question_text = _strip_answer_space_runs(question.text)
        ...
        prompt = json.dumps(
            {
                "task": (
                    ...
                    "labels, or other decorative text. Keep those items in supporting-material or "
...
def _materialize_parser_output(
    ...
    *,
    ...
    for sequence, candidate in enumerate(output.questions, start=1):
        ...
        stem_lines = [working_allowed[item] for item in reconciled_stem_ids]
        if not stem_lines:
        ...
        stem_text = _source_text(stem_lines)
        if unresolved_stem_ids:
        ...
        for option_sequence, option in enumerate(candidate.option_candidates, start=1):
            ...
            if option_source_ids.intersection(used_stem_lines | used_option_lines):
                    ...
                    ExtractionReconciliationWarning(
                        code=(
                            ...
                            if _TECHNICAL_TOKEN.search(
                                option.candidate_text + _source_text(option_lines)
                            )
            ...
            options.append(
                ExtractedQuestionOption(
                    ...
                    option_text=_option_text(option_lines, option.label),
                    sequence=option_sequence,
                    page_number=option_lines[0].page_number,
                    confidence=min(float(line.confidence or 0.0) for line in option_lines),
                    geometry=candidate_geometry(option.geometry) or _union_geometry(option_lines),
        ...
        questions.append(
            ExtractedQuestion(
                ...
                review_status=(
                    QuestionReviewStatus.NEEDS_REVIEW
                    if (
                        candidate.question_type is QuestionType.UNKNOWN
