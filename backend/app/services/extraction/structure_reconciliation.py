...
def reconcile_structure_candidates(
    *,
    ...
    local_option_source_ids = {
        source_line_id
        for question in local_questions
        for option in question.options
        for source_line_id in option.source_line_ids
    }
    claimed_question_source_ids: set[str] = set()
    ...
    for visual in visual_questions:
        ...
        if local is None:
            ...
            normalized_label = visual.number_label.casefold()
            # AI can occasionally reinterpret a visible MCQ option or a second
            # view of the same source region as a new question.  Deterministic
            # source-backed option ownership and already-claimed question spans
            # are stronger evidence than an unmatched visual candidate.  Keep a
            # review warning, but never materialize a duplicate/fake question.
            if (
                visual_source_ids.intersection(local_option_source_ids)
                or visual_source_ids.intersection(claimed_question_source_ids)
                or normalized_label in claimed_labels
            ):
                warnings.append(_warning("QUESTION_BOUNDARY_MISMATCH", visual))
                continue
            reconciled.append(replace(visual, review_status=QuestionReviewStatus.NEEDS_REVIEW))
            claimed_question_source_ids.update(visual.source_line_ids)
            claimed_labels.add(normalized_label)
            if visual.local_key:
        ...
        claimed_question_source_ids.update(proposed.source_line_ids)
        claimed_labels.add(proposed.number_label.casefold())

    for local in remaining:
        reconciled.append(local)
        claimed_question_source_ids.update(local.source_line_ids)
        claimed_labels.add(local.number_label.casefold())
