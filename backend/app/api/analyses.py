...
def _uploaded_pdf_path(
    ...
    label = "Exam" if file_type is UploadedFileType.EXAM else "Course Specification"
    if uploaded is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"{label} PDF not found."
        )
...
def _exam_pdf_path(analysis: Analysis, db: Session, settings: Settings) -> Path:
    return _uploaded_pdf_path(analysis, db, settings, UploadedFileType.EXAM)


def _tp153_pdf_path(analysis: Analysis, db: Session, settings: Settings) -> Path:
    return _uploaded_pdf_path(analysis, db, settings, UploadedFileType.TP153)
...
@router.get("/{analysis_id}/files/tp153/content", response_class=FileResponse)
def get_course_specification_pdf_content(
    analysis: Annotated[Analysis, Depends(get_owned_analysis)],
    db: Annotated[Session, Depends(get_db)],
    settings: Annotated[Settings, Depends(get_settings)],
) -> FileResponse:
    path = _tp153_pdf_path(analysis, db, settings)
    return FileResponse(
        path,
        media_type="application/pdf",
        headers={
...
@router.get("/{analysis_id}/files/tp153/pages/{page_number}/image")
def get_course_specification_pdf_page_image(
    analysis: Annotated[Analysis, Depends(get_owned_analysis)],
    db: Annotated[Session, Depends(get_db)],
    settings: Annotated[Settings, Depends(get_settings)],
    page_number: int,
...
def list_analysis_document_references(
    ...
    for row in rows:
        ...
        contextual_candidates = [
            item for item in active_candidates if not item.exact_label_match
        ]
        selected_candidates = [item for item in active_candidates if item.selected]
        resolution_status = (
            ...
            if len(selected_candidates) == 1
                ...
                if (
                    ...
                    or (
                        row.normalized_target_label.endswith(":unlabeled")
                        and len(contextual_candidates) > 1
                    )
                )
