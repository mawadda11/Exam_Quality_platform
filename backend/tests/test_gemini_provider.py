...
def test_remote_protocol_failure_is_availability_and_sanitized() -> None:
    provider, _ = make_provider(
        exc=httpx.RemoteProtocolError("private remote protocol detail")
    )
    ...
    assert caught.value.kind is AiFailureKind.AVAILABILITY
    assert "private remote protocol detail" not in str(caught.value)
...
def test_provider_removes_risky_schema_keywords_from_request_copy() -> None:
    schema = {
        "type": "object",
        "properties": {
            "id": {
    ...
    provider, models = make_provider(
        response=FakeResponse('{"id":"00000000-0000-0000-0000-000000000000"}')
    )

    provider.generate_structured(system="System", prompt="Prompt", schema=schema)

    sent_schema = models.calls[0]["config"].response_json_schema
    assert sent_schema["properties"]["id"] == {"type": "string"}
    assert sent_schema["required"] == ["id"]
