...
def test_owner_can_preview_uploaded_course_specification_pdf(client: TestClient) -> None:
    email = "course-spec-preview-owner@example.edu"
    created = client.post("/api/v1/analyses", json=PAYLOAD, headers=auth_header(email))
    assert created.status_code == 201
    analysis_id = str(created.json()["id"])
    uploaded = client.post(
    ...
    content = client.get(
        f"/api/v1/analyses/{analysis_id}/files/tp153/content",
        headers=auth_header(email),
    )
    ...
    page = client.get(
        f"/api/v1/analyses/{analysis_id}/files/tp153/pages/1/image",
        headers=auth_header(email),
    )
    assert page.status_code == 200, page.text
    assert page.headers["content-type"].startswith("image/png")
    ...
    intruder = client.get(
        f"/api/v1/analyses/{analysis_id}/files/tp153/pages/1/image",
        headers=auth_header("course-spec-preview-intruder@example.edu"),
    )
    assert intruder.status_code == 404
