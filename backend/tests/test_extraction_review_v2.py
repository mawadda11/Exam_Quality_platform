...
def _three_child_hierarchical_result(
    *,
    parent_marks: float | None,
    child_marks: tuple[float | None, float | None, float | None],
) -> ExtractionResult:
    base = _result()
    parent = replace(
        base.questions[0],
    ...
    children = [
        replace(
            parent,
            number_label=f"Q3({label})",
            text=f"Subquestion {label}",
            parent_number_label="Q3",
            local_key=f"P1-Q3-{label}",
            parent_local_key="P1-Q1",
            marks=marks,
            sequence=index + 1,
        )
        for index, (label, marks) in enumerate(
            zip(("a", "b", "c"), child_marks, strict=True)
        )
    ]
    return replace(base, questions=[parent, *children], reconciliation_warnings=[])
...
def test_single_unknown_child_mark_is_inferred_from_parent_remainder(
    client: TestClient,
    db_engine: Engine,
) -> None:
    analysis_id = _analysis(client, "review-single-remainder@example.edu")
    with Session(db_engine) as session:
        analysis = session.get(Analysis, analysis_id)
        assert analysis is not None
        persist_extraction_result(
            session,
            analysis.id,
            _three_child_hierarchical_result(
                parent_marks=9,
                child_marks=(3, 3, None),
            ),
        )
        analysis.state = ProcessingStage.REVIEW_READY
        materialize_initial_review_revision(session, analysis.id)
        ...
        response = get_extraction_review(session, analysis)
        child_c = next(
            item for item in response.snapshot.questions if item.number_label == "Q3(c)"
        )
        assert child_c.marks == 3
        assert response.can_confirm is True
        ...
        persisted_child_c = session.execute(
            select(Question).where(
                Question.analysis_id == analysis_id,
                Question.number_label == "Q3(c)",
            )
        ).scalar_one()
        assert persisted_child_c.marks == 3


def test_multiple_unknown_children_are_not_evenly_distributed(
    client: TestClient,
    db_engine: Engine,
) -> None:
    analysis_id = _analysis(client, "review-no-distribution@example.edu")
    with Session(db_engine) as session:
        analysis = session.get(Analysis, analysis_id)
        assert analysis is not None
        persist_extraction_result(
        ...
        response = get_extraction_review(session, analysis)
        marks_by_label = {item.number_label: item.marks for item in response.snapshot.questions}
        assert marks_by_label["Q3(a)"] == 3
        assert marks_by_label["Q3(b)"] is None
        assert marks_by_label["Q3(c)"] is None
        assert response.can_confirm is True


def test_known_child_marks_exceeding_parent_total_does_not_block_extraction_confirmation(
    client: TestClient,
    db_engine: Engine,
) -> None:
    analysis_id = _analysis(client, "review-over-parent@example.edu")
    with Session(db_engine) as session:
        analysis = session.get(Analysis, analysis_id)
        assert analysis is not None
        persist_extraction_result(
...
def test_verified_parent_child_marks_mismatch_is_an_analysis_finding_not_review_blocker(
    client: TestClient,
    db_engine: Engine,
) -> None:
    analysis_id = _analysis(client, "review-marks-finding@example.edu")
    with Session(db_engine) as session:
        analysis = session.get(Analysis, analysis_id)
        assert analysis is not None
        persist_extraction_result(
