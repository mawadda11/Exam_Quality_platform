...
from pdf_fixtures import (
    ...
    build_split_instruction_exam_pdf,
...
def test_recovers_split_general_instruction_block_without_admin_prose(tmp_path: Path) -> None:
    pdf_path = _write(tmp_path, "split-instructions.pdf", build_split_instruction_exam_pdf())

    result = PdfPlumberExamExtractor().extract(pdf_path)

    instructions = [e for e in result.evidence if e.evidence_type == "instructions"]
    assert len(instructions) == 1
    text = instructions[0].extracted_text
    assert "Answer all questions" in text
    assert "Show calculation steps" in text
    assert "Unauthorized materials" in text
    assert "Administrative fixture note" not in text
